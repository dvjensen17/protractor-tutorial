/**
 * protractor.conf.js (Protractor Configuration File - For Javascript End to End Testing Framework)
 *
 * Run End to End Frontier-Tree Protractor Test Specifications
 *
 * RUNNING LOCALLY
 * ./bin/start-protractor.sh (STARTS local protractor selenium server instance)
 * ./bin/e2e.sh (Run the full set of tests against your local frontier-tree dev code localhost:5000 and integration.familysearch.org reference)
 * npm run e2e (same as above but using npm)
 *
 * RUNNING REMOTELY
 * ./bin/e2e-remote.sh (acceptance test script used in frontier-tree build pipeline)
 * npm run e2e-remote (same as above but using npm)
 *
 ...
 *
 *
 * joelthornton  f78ac5ed-29c4-440f-a19b-8093b6f8a697
 *
 */

const q = require('q');
const testConfig = (new (require('qa-shared-base/lib/config/testConfig.js'))(__dirname, './config.json')).getConfig();
...
exports.config = {
  ...
  //  name: "FamilySearch Frontier Tree Acceptance test",
  //
  //  proxy: {
  //    proxyType: "PAC",
  ...
  experimentList: testConfig.experimentList,
  ...
  mochaOpts: testConfig.mochaOpts,

  // set 'notEC' to true in config for suites that are not running on Electric Flow
  notEC: testConfig.notEC,
...
if (testConfig.multiCapabilities) {
  exports.config.multiCapabilities = testConfig.multiCapabilities;
} else {
  exports.config.capabilities = testConfig.capabilities;
}
